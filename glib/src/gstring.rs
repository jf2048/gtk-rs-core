...
// rustdoc-stripper-ignore-next
/// Helper for formatting a GString on the stack.
#[repr(transparent)]
struct GStringBuilderInline(ffi::GString);

impl Default for GStringBuilderInline {
    fn default() -> Self {
        Self(ffi::GString {
            str: ptr::null_mut(),
            len: 0,
...
impl fmt::Write for GStringBuilderInline {
    #[inline]
    fn write_str(&mut self, s: &str) -> fmt::Result {
        unsafe {
            ffi::g_string_append_len(
                &mut self.0,
                s.as_ptr() as *const _,
                s.len().try_into().map_err(|_| fmt::Error)?,
            );
        }
        Ok(())
    }
    ...
    fn write_char(&mut self, c: char) -> fmt::Result {
        unsafe {
            ffi::g_string_append_unichar(&mut self.0, c.into_glib());
        }
        Ok(())
    }
}

impl GStringBuilderInline {
    fn into_gstring(mut self) -> GString {
        if self.0.str.is_null() {
            debug_assert_eq!(self.0.allocated_len, 0);
            GString::new()
        } else {
            debug_assert!(self.0.allocated_len > 0 && self.0.len > 0);
            let ptr = mem::replace(&mut self.0.str, ptr::null_mut());
...
impl Drop for GStringBuilderInline {
    fn drop(&mut self) {

...
impl ListStore<Object> {
    #[doc(alias = "g_list_store_new")]
    pub fn new_dynamic(item_type: glib::types::Type) -> Self {
...
mod tests {
    ...
    fn type_checking() {
        ...
        object.downcast_ref::<ListStore<Object>>().unwrap();
        ...
        object_model.downcast_ref::<ListStore<Object>>().unwrap();
        object_model.downcast_ref::<ListModel<File>>().unwrap();
        object_model.downcast_ref::<ListStore<File>>().unwrap();

        let object_store = store.upcast_ref::<ListStore<Object>>();
        object_store.upcast_ref::<ListModel<Object>>();
        object_store.dynamic_cast_ref::<ListModel<File>>().unwrap();
        object_store.downcast_ref::<ListStore<File>>().unwrap();

        let value = store.to_value();
        value.get::<ListStore<File>>().unwrap();
        value.get::<ListStore<Object>>().unwrap();
        value.get::<Option<ListStore<Object>>>().unwrap().unwrap();
        ...
        let dynamic_store = ListStore::new_dynamic(File::static_type());
        dynamic_store.upcast_ref::<ListModel<Object>>();
        dynamic_store.dynamic_cast_ref::<ListModel<File>>().unwrap();
        dynamic_store.downcast_ref::<ListStore<File>>().unwrap();
        assert!(dynamic_store
            .dynamic_cast::<ListModel<Application>>()
            .is_err());
    }
